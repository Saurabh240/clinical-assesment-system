...
const handleSignup = async (e) => {
  ...
  if (!name || !email || !password || !confirmPassword) {
    setError({ type: "frontend", message: "All fields are required." });
    return;
  }

  if (password.length < 8) {
    setError({ type: "frontend", message: "Password must be at least 8 characters long." });
    return;
  }

  if (password !== confirmPassword) {
    setError({ type: "frontend", message: "Password and Confirm Password must match." });
    return;
  }
  ...
  try {
    ...
    navigate("/pharmacy-select"); // make sure route path matches exactly
  } catch (err) {
    if (!err.response) {
      setError({ type: "network", message: "Unable to connect to server. Please try again later." });
    } else {
      setError({
        type: "backend",
